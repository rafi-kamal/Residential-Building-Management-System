...
create index ix_bill_notification_bill_4 on bill_notification (bill_id);
alter table maintenance_task_notification add constraint fk_maintenance_task_notificati_5 foreign key (receiver_id) references user_account (id) on delete restrict on update restrict;
create index ix_maintenance_task_notificati_5 on maintenance_task_notification (receiver_id);
alter table maintenance_task_notification add constraint fk_maintenance_task_notificati_6 foreign key (maintenance_task_id) references maintenance_task (id) on delete restrict on update restrict;
create index ix_maintenance_task_notificati_6 on maintenance_task_notification (maintenance_task_id);
alter table message add constraint fk_message_thread_7 foreign key (THREAD_ID) references thread (internal_id) on delete restrict on update restrict;
create index ix_message_thread_7 on message (THREAD_ID);
alter table thread add constraint fk_thread_sender_8 foreign key (sender_id) references user_account (id) on delete restrict on update restrict;
create index ix_thread_sender_8 on thread (sender_id);
alter table thread add constraint fk_thread_receiver_9 foreign key (receiver_id) references user_account (id) on delete restrict on update restrict;
